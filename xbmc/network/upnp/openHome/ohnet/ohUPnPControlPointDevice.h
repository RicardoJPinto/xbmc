...
#include "network/upnp/openHome/ohnet/UPnPIcon.h"
...
public:
  ...
  const std::vector<CUPnPIcon>& GetIcons() const { return m_icons; }
  std::string GetIconUrl() const;
  std::string GetIconUrl(const std::string &mimetype) const;

protected:
  virtual bool setDescription(const std::string &descriptionUrl, const std::string &descriptionXml);
  virtual bool deserialize(const std::string &descriptionXml);
