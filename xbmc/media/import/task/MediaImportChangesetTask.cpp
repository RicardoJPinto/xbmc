...
CMediaImportChangesetTask::CMediaImportChangesetTask(const CMediaImport &import, IMediaImportHandler *importHandler, const CFileItemList &localItems, const ChangesetItems &retrievedItems)
...
bool CMediaImportChangesetTask::DoWork()
{
  ...
  SetProgressText(StringUtils::Format(g_localizeStrings.Get(37038).c_str(), MediaTypes::GetPluralLocalization(m_import.GetMediaType()).c_str()));
