  ...
  : CJobQueue(false, 1, CJob::PRIORITY_LOW_PAUSABLE)
{
  ...
  // TODO
  RegisterMediaImportHandler(new CArtistImportHandler());
  RegisterMediaImportHandler(new CAlbumImportHandler());
  RegisterMediaImportHandler(new CSongImportHandler());
}
...
void CMediaImportManager::RegisterImportRepository(IMediaImportRepository* importRepository)
{
...
bool CMediaImportManager::UnregisterImportRepository(const IMediaImportRepository* importRepository)
{
  if (importRepository == NULL)
    return false;
...
void CMediaImportManager::Import(const CMediaImportSource &source, bool automatically /* = false */)
{
  ...
  // get all the imports for the source
  std::vector<CMediaImport> imports = GetImportsBySource(source.GetIdentifier());
  ...
  if (!imports.empty())
    CLog::Log(LOGWARNING, "CMediaImportManager: failed to import media items for %zu imports from %s", imports.size(), source.GetIdentifier().c_str());
}
