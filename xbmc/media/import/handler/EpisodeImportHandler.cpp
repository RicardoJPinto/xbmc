...
bool CEpisodeImportHandler::HandleImportedItems(CVideoDatabase &videodb, const CMediaImport &import, const CFileItemList &items, IMediaImportTask *task)
{
  ...
  for (int i = 0; i < newItems.Size(); i++)
  {
    ...
    if (episode->m_iIdShow <= 0)
    {
      ...
      tvshow.m_iDbId = tvshow.m_iIdShow = videodb.SetDetailsForTvShow(tvshowPaths, tvshow, CGUIListItem::ArtMap(), std::map<int, std::map<std::string, std::string> >());
      ...
      tvshowItem->SetSource(pItem->GetSource());
      tvshowItem->SetImportPath(pItem->GetImportPath());
    ...
    episode->m_iDbId = videodb.SetDetailsForEpisode(pItem->GetPath(), *episode, pItem->GetArt(), episode->m_iIdShow);
