...
void CVideoDatabase::CreateAnalytics()
{
  ...
  m_pDS->exec("CREATE TRIGGER delete_tvshow AFTER DELETE ON tvshow FOR EACH ROW BEGIN "
              ...
              "DELETE FROM importlinks WHERE idMedia=old.idMovie AND media_type='tvshow'; "
              "END");
  m_pDS->exec("CREATE TRIGGER delete_musicvideo AFTER DELETE ON musicvideo FOR EACH ROW BEGIN "
              ...
              "DELETE FROM importlinks WHERE idMedia=old.idMovie AND media_type='musicvideo'; "
              "END");
  m_pDS->exec("CREATE TRIGGER delete_episode AFTER DELETE ON episode FOR EACH ROW BEGIN "
              ...
              "DELETE FROM importlinks WHERE idMedia=old.idMovie AND media_type='episode'; "
              "END");
  m_pDS->exec("CREATE TRIGGER delete_season AFTER DELETE ON seasons FOR EACH ROW BEGIN "
              ...
              "DELETE FROM importlinks WHERE idMedia=old.idMovie AND media_type='season'; "
              "END");
  ...
  try
  {
    ...
    if (mediaType == MediaTypeMovie)
    {
      ...
      strSQL = PrepareSQL("SELECT movie.idMovie, movie.idFile, path.idPath, movie.c%02d FROM movie "
                          ...
                          "JOIN importlinks ON importlinks.idMedia = movie.idMovie AND importlinks.media_type = 'movie' "
                          "JOIN imports ON imports.idImport = importlinks.idImport AND imports.media_type = 'movie' "
    ...
    else if (mediaType == MediaTypeTvShow)
    {
      ...
      strSQL = PrepareSQL("SELECT tvshow.idShow, path.idPath, tvshow.c%02d FROM tvshow "
                          ...
                          "JOIN importlinks ON importlinks.idMedia = tvshow.idShow AND importlinks.media_type = 'tvshow' "
                          "JOIN imports ON imports.idImport = importlinks.idImport AND imports.media_type = 'tvshow' "
                          "WHERE imports.idImport = %d "
                          "GROUP BY tvshow.idShow", VIDEODB_ID_TV_TITLE, idImport);
      m_pDS2->query(strSQL);
      total = m_pDS2->num_rows();
      index = 0;
      while (!m_pDS2->eof())
      {
        ...
        int idShow = m_pDS2->fv(0).get_asInt();

        // get all paths belonging to the tvshow
        std::set<int> tvshowPaths;
        if (!GetPathsForTvShow(idShow, tvshowPaths))
        {
          if (standalone)
            RollbackTransaction();
          return false;
        ...
        // only delete the tvshow if the import path is the only path
        tvshowPaths.erase(m_pDS2->fv(1).get_asInt());
        if (!tvshowPaths.empty())
          DeleteTvShow(idShow, false, false);
    ...
    else if (mediaType == MediaTypeSeason)
    {
      ...
      strSQL = PrepareSQL("SELECT seasons.idSeason, seasons.season, tvshow.c%02d FROM seasons "
                          "JOIN importlinks ON importlinks.idMedia = seasons.idSeason AND importlinks.media_type = 'season' "
                          "JOIN imports ON imports.idImport = importlinks.idImport AND imports.media_type = 'season' "
    ...
    else if (mediaType == MediaTypeEpisode)
    {
      ...
      strSQL = PrepareSQL("SELECT episode.idEpisode, episode.idFile, path.idPath, episode.c%02d, tvshow.c%02d FROM episode "
                          "JOIN files ON files.idFile = episode.idFile "
                          "JOIN path ON path.idPath = files.idPath "
                          "JOIN importlinks ON importlinks.idMedia = episode.idEpisode AND importlinks.media_type = 'episode' "
                          "JOIN imports ON imports.idImport = importlinks.idImport AND imports.media_type = 'episode' "
      ...
      while (!m_pDS2->eof())
      {
        if (progress != NULL)
        {
          ...
          std::string showTitle = m_pDS2->fv(4).get_asString();
          if (!showTitle.empty())
            progress->SetLine(1, showTitle + ": " + m_pDS2->fv(3).get_asString()); // TODO: localization
          else
            progress->SetLine(1, m_pDS2->fv(3).get_asString()); // TODO: localization
          progress->SetPercentage((index * 100) / total);
          progress->Progress();
        }

        DeleteEpisode(m_pDS2->fv(0).get_asInt(), false);
    ...
    else if (mediaType == MediaTypeMusicVideo)
    {
      ...
      strSQL = PrepareSQL("SELECT musicvideo.idMVideo, musicvideo.idFile, path.idPath, musicvideo.c%02d FROM musicvideo "
                          ...
                          "JOIN path ON path.idPath = files.idPath "
                          "JOIN importlinks ON importlinks.idMedia = musicvideo.idMVideo AND importlinks.media_type = 'musicvideo' "
                          "JOIN imports ON imports.idImport = importlinks.idImport AND imports.media_type = 'musicvideo' "
