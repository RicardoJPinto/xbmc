...
bool CAddonInstaller::InstallFromZip(const std::string &path)
{
  ...
  if (!CDirectory::GetDirectory(zipDir, items) || items.Size() != 1 || !items[0]->m_bIsFolder)
  {
    CEventLog::GetInstance().AddWithNotification(
      EventPtr(new CNotificationEvent(EventLevelWarning, 24045, StringUtils::Format(g_localizeStrings.Get(24092).c_str(), path.c_str()))), false);
    return false;
  }
...
void CAddonInstallJob::ReportInstallError(const std::string& addonID, const std::string& fileName, const std::string& message /* = "" */)
{
  ...
  {
    activity = EventPtr(new CNotificationEvent(EventLevelError, 24045, !msg.empty() ? msg : StringUtils::Format(g_localizeStrings.Get(24092).c_str(), fileName.c_str())));
...
void CAddonUnInstallJob::OnPostUnInstall()
{
  ...
  if (!database.Open() || !database.GetAddon(m_addon->ID(), addon) || addon == NULL)
    addon = m_addon;
  CEventLog::GetInstance().Add(EventPtr(new CAddonManagementEvent(addon, 24093)));
